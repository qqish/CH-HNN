...
parser.add_argument(
    '--root_path', default='exp/golden20231102_gradual', type=str)
...
parser.add_argument('--task_times', default=99, type=int)
...
parser.add_argument('--seed', default=3722, type=int)
...
def get_permutation_index(times):
    ...
    utils.show_task_similarity(index[-args.test_task_num:, :], args.root_path)
    return index
...
def get_gate_net():
    net = aux_net(inputs_size=args.inputs_size, hidden_size_list=[
                  32, 32, 10], channels=args.channels, n_value=args.n_value).cuda()
    return net
...
def train_ann_task(gate_net, data_set, permutation_index_list):
    ...
    optimizer_aux = optim.Adam(
        gate_net.parameters(), lr=args.lr_s1)
    ...
    for batch_idx, (inputs, targets) in tqdm(enumerate(train_set)):
        ...
        for i in range(num):
            regularizer_list.append(utils.get_conjugate_loss(
                gate_1[i], gate_2[i], permutation_index_list[task_index_1], permutation_index_list[task_index_2]).unsqueeze(0))
