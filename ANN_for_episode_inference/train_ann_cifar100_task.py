...
def generate_task_loader(task_index_list, class_num_per_task=1, train=True):
    ...
    if (train == True):
        dataset_ = dataset.Dataset(
            'data/CIFAR100/train/', class_list=class_list)
    else:
        dataset_ = dataset.Dataset(
            'data/CIFAR100/test/', class_list=class_list)
...
def train_ann_task(gate_net):
    ...
    task_similarity_matrix = np.zeros((100, 100))

    for i in range(100):
        for j in range(100):
            if (coarse_labels[i] == coarse_labels[j]):
                task_similarity_matrix[i, j] = 1
    ...
    task_index_list = np.arange(100)
    np.random.shuffle(task_index_list)

    train_set = generate_task_loader(
        task_index_list[:args.train_task_num], train=True)
    test_set = generate_task_loader(
        task_index_list[-args.test_task_num:], train=False)
