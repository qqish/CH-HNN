...
def get_conjugate_loss(gate_1, gate_2, index_1, index_2, l_order):
    loss_list = []
    Y = get_task_similarity_matrix(index_1, index_2, gate_1.size(0))
    for i in range(gate_1.size(-1)):
        sim_i = get_cosine_similarity_1dim(
            gate_1[:, :, i], gate_2[:, :, i]).unsqueeze(0)
        loss_list.append((torch.abs(sim_i-Y)**l_order).mean())
...
def createHyperparametersFile(args):
    path = args.root_path
