...
parser.add_argument('--lr_s1', default=0.00001,
                    type=float, help='learning rate')
parser.add_argument(
    '--dataset', default='cifar100', type=str)
parser.add_argument('--batch_size', default=200, type=int)
parser.add_argument('--train_task_num', default=100, type=int)
parser.add_argument('--train_task_ratio', default=0.2, type=float)
...
parser.add_argument('--l_order', default=4.0, type=float)
...
parser.add_argument(
    '--pretrain_path', default='exp/imagenet2cifar100_800class_pretrain_64ch', type=str)
...
def get_gate_net():
    ...
    net.load_state_dict(torch.load(args.pretrain_path+'/models/gate_net.pth'))
...
def train_ann_task(gate_net):
    if (args.dataset == 'tinyimagenet'):
        task_similarity_matrix = np.load(
            '../class_incremental_learning/cor/tinyimagenet_vitl14.npy')
    elif (args.dataset == 'imagenet'):
        task_similarity_matrix = np.load(
            '../class_incremental_learning/cor/imagenet_vitl14.npy')
    elif (args.dataset == 'cifar100'):
        task_similarity_matrix = np.load(
            '../class_incremental_learning/cor/cifar100_vitl14.npy')

    task_similarity_matrix = torch.from_numpy(task_similarity_matrix).cuda()
    # task_similarity_matrix[task_similarity_matrix < 0.95] = 0
    ...
    for k, v in gate_net.named_parameters():
        if ('linear' in k):
            v.requires_grad = False

    optimizer_aux = optim.Adam(filter(lambda p: p.requires_grad, gate_net.parameters(
    )), lr=args.lr_s1, betas=(0.0, 0.999), weight_decay=0.0001)
