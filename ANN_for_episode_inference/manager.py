                ...
                for _, class_num in enumerate(class_num_all):
                    for _, c in enumerate(channels_all):
                        for _,meta in enumerate(meta_all):
                            ...
                            cmd='python train_ann_cifar100_continued.py '
                            cmd+='--gpu {} '.format(gpu)
            ...
            for _, epochs in enumerate(epochs_all):
                for _, class_num in enumerate(class_num_all):
                    for _, c in enumerate(channels_all):
                        ...
                        gpu=gpu_list[cnt%len(gpu_list)]
                        cmd='python train_ann_cifar100_enhanced.py '
                        cmd+='--gpu {} '.format(gpu)
                        cmd+='--dataset "tinyimage" '
                        cmd+='--lr_s1 {} '.format(lr)
                        cmd+='--stage_1_epoch {} '.format(epochs)
                        ...
                        cmd+='--root_path "exp_tinyimage_random200_epochs_60/exp_gate64/" '.format(task_num[0], neuron, epochs, c, lr)
                        cmd+='--n_dim {} '.format(neuron)
                        cmd+='--channels {} '.format(c)
            ...
            for _, epochs in enumerate(epochs_all):
                for _, class_num in enumerate(class_num_all):
                    for _, c in enumerate(channels_all):
                        ...
                        gpu=gpu_list[cnt%len(gpu_list)]
                        cmd='python train_ann_cifar100_task.py '
                        cmd+='--gpu {} '.format(gpu)
